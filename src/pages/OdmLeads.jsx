...
const OdmLeads = () => {
    ...
    // Filter for ODM leads (product inquiries) - include leads without leadType for backwards compatibility
    const leads = data?.data?.filter(lead => !lead.leadType || lead.leadType === 'ODM') || [];
    ...
    const handleLeadAdded = () => {
        // RTK Query will automatically refetch and update
    };
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">ODM Leads</h1>
                    <p className="text-sm text-gray-500 mt-1">Product catalog inquiries (buyers interested in our designs)</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                {leads.length === 0 ? (
                    <div className="text-center py-12 text-gray-500">
                        No ODM leads found. Product inquiries from "Send Inquiry" will appear here.
                    </div>
                ) : (
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-medium text-gray-500 text-sm">Date & Time</th>
                            ...
                            {leads.map((lead) => (
                                <tr key={lead._id} className="hover:bg-gray-50 group">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        ...
                                        {lead.quantity && <div className="text-xs text-gray-400">{lead.quantity} pcs</div>}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        {lead.phone ? (
